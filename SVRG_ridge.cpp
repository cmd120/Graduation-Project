...
void SVRG_ridge(VectorXd &w, const MatrixXd &Xt, VectorXd y, const MatrixXd &XtTest, \
    ...
    for (int i = 0; i < pass; i++) {
        flag = batchSize?SVRG_RidgeInnerLoopBatchDense(w, Xt, y, XtTest, yTest, wtilde, G, lambda, maxIter, nSamples, nVars, pass, a, b, gamma, batchSize, maxRunTime):\
                            SVRG_RidgeInnerLoopSingleDense(w, Xt, y, XtTest, yTest, wtilde, G, lambda, maxIter, nSamples, nVars, pass, a, b, gamma, maxRunTime);
        if (flag) {
            break;
        }
    }
