...
gradients) % g(n,1) - previous derivatives of loss % maxIter - scalar maximal
iterations % batchSize - mini-batch size m % pass - used to determine #iteration
  ...
  for (i = 0; i < maxIter; i++) {
    ...
    if ((i + 1) % maxIter == maxIter * epochCounter / PRINT_FREQ) {
      ...
      if (telapsed >= maxRunTime) {
        std::cout << "total telapsed: " << telapsed << std::endl;
        return 1;
      }
    }
  }
...
int IAGA_LogisticInnerLoopSingle(
  ...
  for (i = 0; i < maxIter; i++) {
    ...
    tmpFactor = eta / c * (tmpGrad - g[idx]);  // @NOTE biased estimator
    ...
    // libmkl_intel_ilp64.a for 64bit integer std::cout << "pass 5" <<
    // std::endl; Step 4: update d and g(idx)
    j = outerStarts[idx];
    for (Eigen::SparseMatrix<double>::InnerIterator it(Xt, idx); it;
         ++it, ++j) {
