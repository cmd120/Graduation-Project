...
void IAG_init(MatrixXd &Xt, VectorXd &w, MatrixXd &XtTest, VectorXd &yTest, \
			double &lambda, double &eta, double &a, double &b, double &gamma,\
			int &maxIter, int &batchSize, int &passes, int &maxRunTime, string &filename, int &datasetNum);

void IAG_init(SparseMatrix<double> &Xt, VectorXd &w, SparseMatrix<double> &XtTest, VectorXd &yTest, \
			double &lambda, double &eta, double &a, double &b, double &gamma,\
			int &maxIter, int &batchSize, int &passes, int &maxRunTime, string &filename, int &datasetNum);

int IAG_LogisticInnerLoopSingle(VectorXd &w, const MatrixXd &Xt, VectorXd &y, \
								const MatrixXd &XtTest, VectorXd &yTest,\
								VectorXd &d, VectorXd &g, double lambda, long maxIter,\
								int nSamples, int nVars, int pass, double a, double b,\
...
int IAG_RidgeInnerLoopBatchDense(VectorXd &w, const MatrixXd &Xt, VectorXd &y, const MatrixXd &XtTest, VectorXd &yTest, VectorXd &d, VectorXd &g, double lambda, long maxIter, int nSamples, int nVars, int pass, double a, double b, double gamma, int maxRunTime, int batchSize);
