...
void SIG_ridge(VectorXd &w, const MatrixXd &Xt, VectorXd y, const MatrixXd &XtTest, \
    ...
    for (int i = 0; i < pass; i++) {
        flag = batchSize?SIG_RidgeInnerLoopBatchDense(w, Xt, y, XtTest, yTest, wtilde, G, lambda, maxIter, nSamples, nVars, pass, a, b, gamma, batchSize, maxRunTime):\
                            SIG_RidgeInnerLoopSingleDense(w, Xt, y, XtTest, yTest, wtilde, G, lambda, maxIter, nSamples, nVars, pass, a, b, gamma, maxRunTime);
        if (flag) {
            break;
        }
    }
